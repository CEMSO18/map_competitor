...
try:
    ...
    results = fetch_data_from_table('societe_info_gameBar_competitor', api_key, conn)

    if results:
        print(json.dumps({'success': True, 'data': results}, ensure_ascii=False))
    else:
        print(json.dumps({'success': False, 'error': 'Aucune donnée trouvée pour la table spécifiée.'}, ensure_ascii=False))
